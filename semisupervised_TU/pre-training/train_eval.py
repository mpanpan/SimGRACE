...
def cross_validation_with_val_set(dataset, args,
                                  model_func,
                                  folds,
                                  epochs,
                                  batch_size,
    ...
    for fold, (train_idx, test_idx, val_idx) in enumerate(
        ...
        vice_model = model_func(dataset).to(device)
        if fold == 0:
            print_weights(model)
        optimizer = Adam(model.parameters(), lr=lr, weight_decay=weight_decay)
        ...
        for epoch in range(1, epochs + 1):
            ...
            print("Memory used:", pynvml.nvmlDeviceGetMemoryInfo(handle).used/ 1024 /1024)
            train_loss, _ = train(
            ...
            with open('logs_simgrace_eta_'+str(args.eta)+'/' + dataset_name + '_' + aug1 + '_' + str(aug_ratio1) + '_'+ aug2 + '_' + str(aug_ratio2) + '_cl_log', 'a+') as f:
                 f.write(str(epoch) + ' ' + str(train_loss))
                 f.write('\n')

            if epoch % 20 == 0:
                 torch.save(model.state_dict(), 'models_simgrace_eta_'+str(args.eta)+'/' + dataset_name + '_' + aug1 + '_' + str(aug_ratio1) + '_'+ aug2 + '_' + str(aug_ratio2) + '_' + str(epoch) + '_' + str(lr) + '_' + str(suffix)  + '.pt')
...
def train(model, vice_model, optimizer, dataset, device, batch_size, aug1, aug_ratio1, aug2, aug_ratio2,args):
    dataset.aug = "none"
    dataset = dataset.shuffle()
    loader1 = DataLoader(dataset, batch_size, shuffle=False)
    model.train()
